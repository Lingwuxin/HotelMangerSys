      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="(item, index) in data" :key="index">
            ...
            <td>
              <!-- 使用 v-if 控制按钮的显示 -->
              <button v-if="item.status !== 'checkout'" @click="() => checkout(item.customerID, index)">退房</button>
              <span v-if="item.status =='checkout'">已退房</span>
            </td>
            <td>
              <button>修改</button>
            </td>
...
onMounted(() => {
  ...
  const checkout = async (customerId: number, index: number) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8080/checkout/${customerId}`);
      // 在页面上移除对应的顾客数据
      await fetchData();
    } catch (error) {
